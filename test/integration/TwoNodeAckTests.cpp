...
TEST_F(TwoNodeAckTests, TwoWayTest)
{
    ...
    bool        node1GotAck;
    std::thread t1([&]() { node1GotAck = node1_.embeddedSF.PublishWait("test-topic", node1DataToSend, std::chrono::milliseconds(1000)); });
    bool        node2GotAck;
    std::thread t2([&]() { node2GotAck = node2_.embeddedSF.PublishWait("test-topic", node2DataToSend, std::chrono::milliseconds(1000)); });
...
TEST_F(TwoNodeAckTests, TwoPublishWaitCalls)
{
    ...
    bool        msg1GotAck;
    std::thread t1([&]() { msg1GotAck = node1_.embeddedSF.PublishWait("topic1", msg1Data, std::chrono::milliseconds(1000)); });
    bool        msg2GotAck;
    std::thread t2([&]() { msg2GotAck = node1_.embeddedSF.PublishWait("topic2", msg2Data, std::chrono::milliseconds(1000)); });
